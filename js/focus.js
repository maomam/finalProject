...
let scaleNotes = Tonal.Scale.get("G#3 major").notes;
...
const shape = (path,i) => {
        ...
        const strnX = TummyPoints[i].x;
        const strnY = TummyPoints[i].y;

        const strgX = NosePoints[i].x;
        const strgY = NosePoints[i].y;
        ...
        path.style.animation = ('focus 7s  5 normal');
};
...
function draw() {
        if (playing) {
                ...
                try {
                       ...
                       const textNode2 = document.createTextNode("We hope you feel more focused.");
